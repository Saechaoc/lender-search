...
- [x] **Structured derogatory-event model** — Validated in ROADMAP Phase 2. `DerogSeasoning` Zod schema with `event_type`, `measurement_anchor`, `base_waiting_months`, `ec_waiting_months`, `post_event_ltv_caps[]`, `re_established_credit_required`, `mortgage_included_in_bk`. FNMA SC#2 foreclosure fixture round-trips through schema. FHA Back-to-Work DEPRECATED encoding completed in Phase 3 (HUD ML 2013-26, sunset 2016-09-30).
- [x] **Append-only `evaluation_event` audit log** — Validated in ROADMAP Phase 3 (audit-log-agency-rule-encoding). Monthly-partitioned table with REVOKE UPDATE/DELETE, RLS+FORCE on parent + 6 forward child partitions, `ruleset_snapshot_id` content-addressed via sha256 of canonical rule bundle (`lib/audit/snapshotId.ts`), `pg_cron` partition auto-create function. Phase 4 evaluator imports the helper unchanged.
- [x] **Hand-authored agency rule sets (FNMA + FHLMC + FHA + VA + USDA scaffold)** — Validated in ROADMAP Phase 3. FNMA 9-row derog matrix with FORECLOSURE 2-row purchase/LCOR split (B3-5.3-07), FHLMC 8-row matrix (§5202.5, 24m foreclosure EC delta vs FNMA), FHA 8 active + 1 DEPRECATED Back-to-Work (HUD 4000.1 + ML 2013-26), VA 8-row (Pamphlet 26-7 — 3 cited BK + 5 `not_applicable` sentinels for cross-agency parity), USDA scaffold-only. Citation discipline enforced (every rule_citation has source_url; A2 URL gate proves zero violations).
- [x] **Agency cascade infrastructure** — Validated in ROADMAP Phase 3. `cascade_review_queue` table + `enqueue_agency_cascade()` AFTER INSERT trigger fans out per-program rows on agency_rule_version supersession. Cross-tenant RLS verified (4 trigger tests + 6 cross-tenant pen tests pass). `lib/cascade/poll.ts` typed stub awaits Phase 6 Vercel Cron + Inngest worker wire-up.
- [x] **2026 FHFA conforming loan limits + high-balance overlays** — Validated in ROADMAP Phase 3. `conforming_loan_limit_version` (year EXCLUDE constraint, daterange, `upper_inf` close-detection) + `_county` (3,235 rows from FHFA FINAL FLAT CSV; 160 high-cost counties; FIPS leading zeros preserved). Optional FK on `program_version`.
...
---
*Last updated: 2026-05-05 after Phase 3 (audit-log-agency-rule-encoding) completion — 8 plans, 9 migrations (0007-0015), 12 new test files, test:rls 62/62 + test:schema 197/197 green; 4 requirements moved to Validated (evaluation_event, agency rule sets, cascade infrastructure, FHFA loan limits). 12 cross-AI REVIEWS blockers + 3 concerns empirically resolved.*
